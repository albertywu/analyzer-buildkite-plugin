...
def main():
  parser = ArgumentParser(description='wrap a shell script invocation with analytics markers')
  ...
  parser.add_argument('--dir', default="%s/artifacts/analysis" % (get_root()),
                      help='directory to deposit analysis results (relative to monorepo root)')
  ...
  if len(rest) == 0:
    ...
    for line in sys.stdin:
      lines += line
    wrapped_cmd(step, type, args, dir, lines)
  else:
    cmd = list2cmdline(rest)
    wrapped_cmd(step, type, args, dir, cmd)
